...
//            };
      ...
      if (tl->lineVisible() || !score()->printing()) {
            ...
            if (hasBeginHook && (isSingleType() || isBeginType())) {
                  ...
                  qreal bx1 = pp1.x() - beginHookWidth;
                  qreal by1 = pp1.y() + beginHookHeight;
                  qreal bx2 = pp1.x();
                  qreal by2 = (beginHookType == HookType::HOOK_90T) ? (pp1.y() - beginHookHeight) : pp1.y();
                  beginHookLine.setPoints(QPointF(bx1, by1), QPointF(bx2, by2));

                  if (beginHookType == HookType::HOOK_90T)
                        points[npoints++] = QPointF(bx1, by1);
                  else  points[npoints++] = QPointF(bx2, by2);
            ...
            if (!backwardsWithText) {
                  ...
                  if (hasEndHook && (isSingleType() || isEndType())) {
                        ...
                        qreal ex1 = textLine.length() + beginHookWidth + l;
                        // Rotation will later handle y-positioning
                        qreal ey1 = (endHookType == HookType::HOOK_90T) ? (0.0 + endHookHeight) : 0.0;
                        qreal ex2 = ex1 + endHookWidth;
                        qreal ey2 = (endHookType == HookType::HOOK_90T) ? (0.0 - endHookHeight) : (0.0 + endHookHeight);
                        endHookLine.setPoints(QPointF(ex1, ey1), QPointF(ex2, ey2));

                        points[npoints++] = QPointF(ex1, ey1);
      ...
      if (tl->lineVisible()) {
            ...
            if (!textLine.isNull()) {
                  ...
                  auto newTranslate = flatLine.p1();
                  lineBB = rotateRectangle(lineBB, newTranslate, angle, -newTranslate);
