...
QColor Element::curColor(bool isVisible, QColor normalColor) const
      ...
      if (MScore::overrideOnlyAllColor)
            ...
            else if (isRehearsalMark())
                  overrideColor = MScore::overrideExpressionTextColor; // TODO: make distinct
            else if (isStaffText() && isStaffTextBase()) {
                  id = toStaffTextBase(this)->tid();
      ...
      if (MScore::highlightMore) {
            ...
            if (isStem()) {
                  ...
                  const NoteDot* dot = toNoteDot(this);
                  if (auto rest = dot->rest())
                        marked = rest->mark();
                  else {
                        const Note* n = dot->note();
            ...
            if (c) {
                  for (auto nn : c->notes()) {
                        marked = nn->mark();
                        if (auto firstNote = nn->firstTiedNote()) {
                              marked = firstNote->mark();
                              }
                        }
                  }
