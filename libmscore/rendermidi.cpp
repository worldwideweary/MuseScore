...
void renderTremolo(Chord* chord, QList<NoteEventList>& ell, int arpeggioDelta, int arpeggio2Delta, int gateTime1, int gateTime2, int vOff1, int vOff2)
      {
      int firstChordNoteCount = (int) chord->notes().size();
      ...
      if (chord->tremoloChordType() == TremoloChordType::TremoloFirstNote) {
            ...
            if (Chord* chord2 = tremolo->chord2()) {
                  ...
                  int secondChordNoteCount = (int)chord2->notes().size();
                  int maxNoteCount = std::max(firstChordNoteCount, secondChordNoteCount);
            ...
            for (int k = 0; k < firstChordNoteCount; ++k) {
...
int renderArpeggio(Chord *chord, QList<NoteEventList> & ell, int prevOnTime)
      ...
      int playTicks = chord->upNote()->playTicks();
...
static QList<NoteEventList> renderChord(Chord* chord, int gateTime, int ontime, int trailtime)
      ...
      int notesCount = (int) chord->notes().size();
