...
QList<int> FretDiagram::pitches() const
      ...
      if (const auto p = part()) {
            if (const auto i = p->instrument()) {
                  ...
                  // Find Dots:
                  for (auto const& item : _dots) {
                        for (auto const& d : item.second) {
                              if (!d.exists())
                                    continue;
                              const int string = (strings() - 1) - item.first;
                  ...
                  // Find Markers:
                  for (auto const& item : _markers) {
                        const int string = (strings() - 1) - item.first;
                        FretItem::Marker marker = item.second;
...
void FretDiagram::draw(QPainter* painter) const
      ...
      for (auto const& i : _dots) {
            for (auto const& d : i.second) {
                  ...
                  // else qDebug() << "Error:" << "no string data from instrument";
      ...
      for (auto const& i : _markers) {
            ...
            if (sd) {
                  const int humanString = (strings() - 1) - string;
                  const int zeroMark = 0;
                  pitches[string] = fretOffset() + sd->getPitch(humanString, zeroMark, staff(), tick());
                  }
      ...
      if (sd && rootExists && (showIntervals() || showNames())) {
            ...
            const QString stepName = tpc2stepName(rootTpc); // [-3, +3]
