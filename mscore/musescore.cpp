...
void MuseScore::populateToggleOptionsMenu()
      ...
      for (const auto s : _toggleOptionsMenuEntries) {
            if (!(strcmp(s, "toggle-options-null") == 0)) {
                  ...
                  if (!strcmp(option, "only-all-color"))
                        choice = MScore::overrideOnlyAllColor;
                  else if (!strcmp(option, "colored-alteration-noteheads"))
                  ...
                  else if (!strcmp(option, "current-system-on-top-skyline"))
                  ...
                  else if (!strcmp(option, "fingering-voicing-layout"))
                        choice = MScore::fingeringTextOmitVoicing;
                  else if (!strcmp(option, "lasso-border"))
                        choice = MScore::lassoBorderEnabled;
                  else if (!strcmp(option, "honor-en-passant-visibility"))
                        choice = MScore::honorEnPassantVisibility;
                  else if (!strcmp(option, "move-cursor-by-beat"))
                        choice = MScore::cursorMoveByBeat;
                  else if (!strcmp(option, "lasso-annotations"))
                        choice = MScore::lassoAnnotations;
                  else if (!strcmp(option, "move-cursor-by-measure"))
                        choice = MScore::cursorMoveByMeasure;
                  else if (!strcmp(option, "move-cursor-to-playback-position"))
                        choice = MScore::selectionFollowsCursor;
                  else if (!strcmp(option, "move-playback-position-to-start"))
                        choice = MScore::cursorResetToStart;
                  else if (!strcmp(option, "mouse-hover"))
                  ...
                  else if (!strcmp(option, "playback-highlight-notes"))
                        choice = MScore::highlightNotes;
                  else if (!strcmp(option, "playback-highlight-rests"))
                        choice = MScore::highlightRests;
                  else if (!strcmp(option, "playback-highlight-lyrics"))
                        choice = MScore::highlightLyrics;
                  else if (!strcmp(option, "playback-highlight-more"))
                        choice = MScore::highlightMore;
                  else if (!strcmp(option, "reset-entry-NewSystemOrCourtesy"))
                        choice = MScore::resetNoteEntryAtSystemOrCourtesy;
                  else if (!strcmp(option, "retain-augmentation-rhythmMode"))
                        choice = MScore::retainAugmentationInRhythmEntry;
                  else if (!strcmp(option, "single-note-selection-color"))
                        choice = MScore::singleNoteSelectionColorEnabled;
                  else if (!strcmp(option, "ties-consider-ledgers"))
                        choice = MScore::tiesAdjustForLedgerLines;
                  else if (!strcmp(option, "ties-uniformity"))
                        choice = MScore::uniformTieAdjustments;
                  else if (!strcmp(option, "upward-fifth-entry"))
                        choice = MScore::noteInputOctaveUpwardFifth;
                  else if (!strcmp(option, "vertical-note-drag-allowed"))
...
bool MuseScore::eventFilter(QObject *obj, QEvent *event)
      ...
      switch(event->type()) {
            ...
            case QEvent::ShortcutOverride:
                  ...
                  if (obj && obj->objectName() == "searchCombo") {
...
MuseScoreApplication* MuseScoreApplication::initApplication(int& argc, char** argv)
      ...
      if (MuseScore::unstable()) {
            ...
            // Separate appName2 will allow multiple instances between other builds
            // using different names, but not this very project
            appName2 = "mscore371";

            // Separate appName will provide distinct .ini file in configuration directory
            appName  = "MuseScore371";
            }
