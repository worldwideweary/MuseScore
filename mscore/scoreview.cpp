...
void ScoreView::moveCursor(const Fraction& tick, bool viaUserNavigation)
      ...
      if (MScore::cursorMoveByBeat) {
            // Note: pedal-line segments activate on point, whereas note events, not being simultaneously
            //       activated, are one "eps" (1/1920 e.g.) tick after, and hence the note events will
            //       not meet the appropriate BeatType:: due to not having that involved in the calculation!
            
            // Objective: Nudge back one eps appropriately to see if note event is "On Beat" to update cursor
            //            for when MOVE CURSOR BY BEAT is enabled.
            // There may be a better way overall in doing this stuff...but for now:
            ...
            if (offByOne.denominator() > 32) {
                  ...
                  offByOne -= eps;
                  offByOne.rnumerator() = (denomDiff > 1) ? (offByOne.numerator() - (denomDiff - 1))
                                                          : (offByOne.rnumerator());
                  }
      ...
      if (MScore::cursorMoveByMeasure) {
            // Highlight entire measure bounding-box akin (cf musescore.com version @ v. 3.6.2)
...
void ScoreView::cmd(const char* s)
      ...
      static const std::vector<ScoreViewCmd> cmdList {
            ...
            {{"playback-position"}, [](ScoreView* cv, const QByteArray&) {
                  bool considerAllTracks = true;
                  bool useNextPosition = true;
                  if (auto cr = cv->chordRestFromCursor(!useNextPosition, considerAllTracks))
                        cv->cmdGotoElement(cr);
                  }},
            ...
            {{"delete"}, [&](ScoreView* cv, const QByteArray&) {
                  ...
                  if (is.noteEntryMode()) {
                        if (!is.cr()) {
                              ...
                              bool isChord = firstEl->isChord();
                              if (isChord) {
                                    auto c = toChord(firstEl);
                                    auto n = c->upNote();
                                    cv->score()->select(n);
                                    }
                              else cv->score()->select(firstEl);
                              is.moveInputPos(seg);
...
void ScoreView::textTab(bool back)
      ...
      const System* currentSys = ot->findMeasureBase() ? ot->findMeasureBase()->system() : nullptr;
      Fraction nextSysEndPos;
      if (currentSys) {
            if (auto m = back ? currentSys->firstMeasure() : currentSys->lastMeasure()) {
                  auto smb = m->findMeasureBase();
                  if (auto nmb = back ? smb->prev() : smb->next()) {
                        if (auto lm = nmb->system()->lastMeasure()) {
                              nextSysEndPos = lm->endTick();
                              }
                        }
                  }
            }
      ...
      if (nextElement && isFingering && op->isNote()) {
            auto n = toNote(op);
            auto c = n->chord();
      ...
      const auto targetSysPos = nn->chord()->measure()->system()->firstMeasure()->tick();
      const bool isFarOut = nextSysEndPos.isNotZero() && (targetSysPos >= nextSysEndPos);
      if (isFarOut) {
            // Stop moving forward if next position is beyond the end of the following system
            if (op->isNote())
...
void ScoreView::cmdAddPedal(HookType beginHook, HookType endHook)
{
      ...
      if (selection.isRange()) {
            ...
            for (int sIdx = selection.staffEnd()-1; sIdx >= selection.staffStart(); prevPart = _score->staff(sIdx)->part(), --sIdx) {
                  auto part = _score->staff(sIdx)->part();
                  ...
                  startSegment = cr1->segment();
                  endSegment = cr2->segment();
                  ...
                  for (auto interval : spanners) {
                        ...
                        if (spanner->isPedal() && (spanner->staffIdx() == sIdx)) {
                  ...
                  _score->cmdAddSpanner(newPedal, sIdx, startSegment, endSegment);
                  haveNewPedal = false;
                  }
      ...
      // [List: Single / Two on the same staff]
            ...
            if (!noteEntryMode()) {
                  ...
                  if (activePedal) {
                        auto sIdx       = activePedal->staffIdx();
                        ...
                        if (newBeginSegment && newEndSegment) {
                              _score->startCmd();
                                 _score->cmdAddSpanner(newPedal, sIdx, newBeginSegment, newEndSegment); // reads fine...
                              _score->endCmd();
                              endEdit();
      ...
      if (el) {
            ...
            if (rhythmMode) {
                  ...
                  if (cl.back()->isChordRest()) {
                        auto nextCR = toChordRest(cl.back());
                        if (auto nextSeg = nextCR->nextSegmentAfterCR(SegmentType::ChordRest)) {
                              nextCR = nextSeg->nextChordRest(nextCR->track());
                              }
                        _score->select(nextCR, SelectType::SINGLE, 0);
